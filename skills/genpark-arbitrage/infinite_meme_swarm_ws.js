...
(async () => {
    try {
        console.log("🚀 INFINITE MEME SWARM ACTIVATED 🚀");
        // Connect directly to the ws url provided by clawdbot
        ...
        while (true) {
            ...
            try {
                ...
                await page.waitForTimeout(2000); // Give React and Turnstile time to breathe
                
                await page.waitForSelector('input[name="firstName"]', { timeout: 15000 });
                
                // Fill using evaluate for exact React state match
                ...
                if (success) {
                    ...
                    // Proceed through preference screen just in case Clerk needs a commit trigger
                    try {
                        await page.waitForTimeout(2000);
                        const preferenceOptions = await page.$$('button');
                        if(preferenceOptions.length > 2) {
                            await preferenceOptions[1].click();
